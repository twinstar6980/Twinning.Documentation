...
Destructive changes are marked in the [change log](../CHANGELOG.md). Users should always use the latest version and only use older versions when project file version migration is required. Only the last distribution is kept in GitHub Release. Historical distributions can be found in my [OneDrive](https://1drv.ms/f/c/2d321feb9cd374ed/Qu1005zrHzIggC2EAAAAAAAAiShIb2wbJMeZsw).
...
2. Trailing commas
	...
	In some editors, trailing commas are treated as a formatting error by default, and users can disable trailing comma output by modifying the `json_format_disable_array_trailing_comma` and `json_format_disable_object_trailing_comma` item to `true` in the `<home>/script/configuration/setting.json` configuration.
...
Before forwarding big-endian files to the tool for processing, users need to modify the `byte_stream_use_big_endian` option in the `<home>/script/configuration/setting.json` configuration file to `true`, or use the `common.utility.update_global_setting` method to change this value at runtime to tell the tool to treat the incoming files as big-endian.
...
> Users can modify the `filter` regular expression of RSB related functions in the script settings file to allow the tool to recognize `*.obb` files as RSB files.
