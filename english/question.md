...
2. Virtual Terminal Control Sequence: Optional, if not supported, it will prevent the program from modifying different types of text with different colors.
	...
	> Users can disable the use of control sequences by modifying the `disable_basic_virtual_terminal_sequence` item to `true` in the `<home>/script/Runner/Runner.json` configuration.
...
2. Trailing commas
	...
	In some editors, trailing commas are treated as a formatting error by default, and users can disable trailing comma output by modifying the `json_format.disable_array_trailing_comma` and `json_format.disable_object_trailing_comma` item to `true` in the `<home>/script/Runner/Runner.json` configuration.
...
> The regular expression for filtering RSB files in `<home>/script/Executor/Implement/popcap.resource_stream_bundle.ts` can be modified to allow the tool to recognize `*.obb` files as RSB files.
