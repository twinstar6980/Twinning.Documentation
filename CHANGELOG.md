...
* `Assistant` 102
	...
	* `Windows` 修复转发器扩展可能导致一些应用在调用右键菜单时崩溃的 BUG 。

* `Assistant Plus` 81

	* 修复转发器扩展可能导致一些应用在调用右键菜单时崩溃的 BUG 。
...
* `Assistant`
	...
	* **`BUG` `Windows`** 由于系统限制，若点击应用推送的系统通知时存在多个应用实例，将会由首个实例处理该通知的调用事件，而不论该通知是否由首个实例自身推送。
...
* `Assistant Plus`
	...
	* **`BUG`** 由于系统限制，若点击应用推送的系统通知时存在多个应用实例，将会由首个实例处理该通知的调用事件，而不论该通知是否由首个实例自身推送。
	...
	* **`BUG`** 转发器扩展 `forwarder` 在某些虚拟机环境下无法生效。
