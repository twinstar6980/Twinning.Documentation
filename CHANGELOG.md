...
* `Assistant`
	...
	* **`OK` `Windows` `Macintosh`** 由于系统通知的机制，若点击应用推送的系统通知时存在多个应用实例，将会由首个实例处理该通知的调用事件，而不论该通知是否由首个实例自身推送。
...
* `Assistant Plus`
	...
	* **`OK`** 由于系统通知的机制，若点击应用推送的系统通知时存在多个应用实例，将会由首个实例处理该通知的调用事件，而不论该通知是否由首个实例自身推送。
	...
	* **`BUG` `Forwarder Extension`** 扩展在某些虚拟机环境下无法生效。
