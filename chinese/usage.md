...
> 主目录内的 `launch_helper.cmd` 脚本可以快速启动 `Modding Worker` ，可以将多个文件拖拽至脚本上并释放，以追加附加参数。
...
* `Resource Forwarder`
	...
	> 主目录内的 `launch_helper_forwarder.cmd` 脚本可以快速开启 `Resource Forwarder` ，可以将多个文件拖拽至脚本上并释放，以追加附加参数。
