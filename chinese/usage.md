...
工具被设计为主要处理外存空间中的文件对象，⌈ **转发** ⌋ 即以文件对象的路径作为附加参数传递给工具。可以通过启动脚本、Android 文件分享功能、Windows Explorer 扩展等方式进行转发。
...
在脚本目录 `+ <home>/script` 中，有些脚本文件拥有配套的配置文件，其名称与对应的脚本文件相同，但以 `.json` 为扩展名。
...
默认情况下，工具在处理一项命令时会根据其类型（主要通过扩展名）筛选出可用功能供用户选择；
...
> 
> `> .\launch.cmd "C:\Users\TwinStar\Desktop\test.pam" "-method" "popcap.pam.decode"`
> 
> 该命令以 `test.pam` 文件的路径作为 `<input>` ，并指定需执行的功能为 `PopCap-PAM 解码` 。执行完毕后，可在桌面看到一个名为 `test.pam.json` 的新文件，即为解码后的 PAM 数据。
...
根据前述，用户可以自行重写主脚本的运行逻辑，但更推荐在已有的配套脚本基础上扩展开发，它已为用户提供了丰富的功能与良好的交互机制。

若要进行脚本扩展开发，请查看本项目的 [Script 模块](https://github.com/twinkles-twinstar/TwinStar.ToolKit/tree/master/Script) 。
