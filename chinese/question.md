...
基于此，工具并不认为 `.obb` 文件是 RSB 文件，如果直接将 `.obb` 文件转发至工具，用户将无法从备选项中看到 RSB 相关功能。如果用户确认 `.obb` 文件存储 RSB 数据包格式时，需要将文件扩展名从 `.obb` 改为 `.rsb` 才可让工具提供 RSB 相关功能选项。

此外，也可创建指向 `.obb` 文件的软链接，并以 `.rsb` 作为链接扩展名，这可以避免重打包后用户还需将 `.rsb` 重命名为 `.obb` 的麻烦。
