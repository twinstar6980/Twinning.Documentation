...
典型的例子是 RTON 文件不以 `RTON` 作为文件起始四字节，虽然 PvZ-2 游戏能正常读取此类文件，但工具无法解码它们，因为 `RTON` 文件不以 `RTON` 作为起始字节被工具认为是文件结构异常。
...
> 可以通过修改 `- <home>/script/Entry/method/popcap.rsb.ts` 中对 RSB 文件进行筛选的正则表达式，以允许工具识别 `*.obb` 文件为 RSB 文件。
...
> 
> 注意：这些 *.rsb.smf 文件经过了一次 ZLib 压缩，需要先用工具对其进行 `PopCap ZLib 解压` 。PvZ-2 中文版项目组是十分愚蠢的，他们的 RSB 无法使用 ZLib 对资源数据进行压缩，却在 APK 中对 RSB 整体进行了压缩，再在初次进入游戏后将其解压至应用外部存储空间中，这造成了严重的存储空间浪费。
